...
bool moveRight = true;         // Flag to indicate the direction of movement
int previousTime = glutGet(GLUT_ELAPSED_TIME);  // Initial time

void updateBlimp(int value) {
    int currentTime = glutGet(GLUT_ELAPSED_TIME);
    float deltaTime = (currentTime - previousTime) / 1000.0;  // Convert to seconds

    // Update the x-coordinate of the ellipse based on the direction of movement and elapsed time
    if (moveRight) {
        blimpX += blimpSpeed * deltaTime;
    }
    else {
        blimpX -= blimpSpeed * deltaTime;
    }
    ...
    glutPostRedisplay();  // Request a redraw to create an animation effect
    glutTimerFunc(16, updateBlimp, 0);  // 60 frames per second (1000 ms / 60 frames = 16.66 ms per frame)
